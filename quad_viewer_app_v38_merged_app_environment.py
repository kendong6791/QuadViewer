...
APP_SEMANTIC_VERSION = "0.38.0"
APP_DISPLAY_FILENAME = os.path.basename(__file__) if "__file__" in globals() else "quad_viewer_app_v38_merged_app_environment.py"
...
def build_summary_records(
    ...
    for seq, fn in enumerate(readable_files, start=1):
        data, _ = load_json(os.path.join(folder, fn))
...
def build_peak_shape_records(readable_files: List[str], folder: str, analyte: Optional[str], parameter: str) -> List[Dict[str, Any]]:
    records: List[Dict[str, Any]] = []
    for seq, fn in enumerate(readable_files, start=1):
        data, _ = load_json(os.path.join(folder, fn))
        aqm = data.get("analyte_quad_map", {}) if isinstance(data, dict) else {}
...
if st.session_state.page == "Dataset":
    ...
    # Folder selection: path box + optional local dialog helper
    ...
    with colB:
        # Best-effort "Browse" for local use (works when Streamlit has desktop access)
    ...
    # Commit folder change
    folder = folder.strip()
    if folder != st.session_state.selected_folder:
        # Folder changed -> reset dependent state
    ...
    # Always show selected folder path (requirement)
    if st.session_state.selected_folder:
        st.caption(f"Selected folder: {st.session_state.selected_folder}")
    ...
    if st.session_state.selected_folder and os.path.isdir(st.session_state.selected_folder):
        ...
        if not files:
            st.info("No .json files found in the selected folder.")
        else:
            st.markdown(
                """
                <style>
            ...
            for seq, fn in enumerate(files, start=1):
                with st.container(border=True):
                    ...
                    if row_cols[1].button(str(injection_id), key=f"open_injection_{fn}", type="tertiary", use_container_width=True):
...
# -----------------------------
    ...
    if not folder or not os.path.isdir(folder):
        st.warning("No valid folder selected. Go to Dataset page first.")
        st.stop()

    files = st.session_state.file_list or list_json_files(folder)
    ds = load_dataset(folder, files)
    unreadable = ds["unreadable"]
    readable_files = [f for f in files if f not in unreadable]
    ...
    st.caption(f"Selected folder: {folder}")
    ...
    ctrl1, ctrl2, ctrl3 = st.columns([1.1, 1.3, 1.3])
...
# -----------------------------
    ...
    records = build_summary_records(readable_files, folder, analyte, parameter)
...
# -----------------------------
    ...
    records = build_peak_shape_records(readable_files, folder, analyte, parameter)
    records_df = pd.DataFrame(records)
...
# -----------------------------
    ...
    if not readable_files:
        st.warning("No readable files available.")
        st.stop()
    ...
    ctrl_trace, ctrl_apex, ctrl_consensus, ctrl_expected = st.columns([1.3, 1.0, 1.1, 1.2])
    with ctrl_trace:
        trace_mode = st.radio(
            "Trace display",
            ["Smoothed", "Raw"],
    ...
    with ctrl_apex:
        show_apex = st.checkbox("Show apex line", value=False)
    with ctrl_consensus:
        show_consensus_rt = st.checkbox("Show consensus RT", value=True)
    with ctrl_expected:
        show_expected_peak = st.checkbox("Show expected peak", value=True)

    st.caption(f"Selected folder: {folder}")

    # Load current injection JSON
    path = os.path.join(folder, st.session_state.selected_file)
    inj, err = load_json(path)
    if inj is None:
        st.error(f"Unable to load JSON: {err}")
        st.stop()
