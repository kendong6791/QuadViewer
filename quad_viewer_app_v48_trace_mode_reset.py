...
APP_SEMANTIC_VERSION = "0.48.0"
APP_DISPLAY_FILENAME = os.path.basename(__file__) if "__file__" in globals() else "quad_viewer_app_v48_trace_mode_reset.py"
...
def render_application_environment_sidebar() -> None:
    ...
    # Keep the application identity visible on every page, even when the
    # environment expander is closed.
    st.sidebar.markdown(
        f"""
        <div style="font-size:0.78rem; color:rgba(49,51,63,0.65); line-height:1.25; margin-bottom:0.4rem;">
            <b>Version:</b> {version_text}<br>
            <b>File:</b> {file_text}
        </div>
        """,
        ...
        if y_line is not None:
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    name="Fitted line",
                    line=dict(color=CHANNEL_STYLES[("analyte", "qualifier")]["dark"], width=2, dash="dash"),
                    hoverinfo="skip",
                )
            )
...
def extract_injection_metadata(data: Optional[dict], filename: str, analyte: Optional[str] = None) -> Dict[str, str]:
    """Extract injection-level display metadata, preferring the selected analyte quad when available."""
    metadata = {
        "Injection ID": os.path.splitext(filename)[0],
        "Sample ID": "",
        "Injection Status": "UNKNOWN",
        "Software Opinion": "UNKNOWN",
        "Human Opinion": "NULL",
    }
    ...
    aqm = data.get("analyte_quad_map", {})
    if not isinstance(aqm, dict):
        return metadata

    quad = aqm.get(analyte, None) if analyte else None
    if not isinstance(quad, dict):
        quad = next((v for v in aqm.values() if isinstance(v, dict)), None)

    if not isinstance(quad, dict):
        return metadata

    metadata["Software Opinion"] = normalize_opinion(quad.get("software_opinion", None))
    metadata["Human Opinion"] = normalize_human_opinion(quad.get("human_opinion", None))

    inj_block = quad.get("injection", {})
    if isinstance(inj_block, dict):
        metadata["Injection Status"] = safe_upper_status(inj_block.get("status", None))

        result = inj_block.get("result", {})
        if isinstance(result, dict):
            metadata["Injection ID"] = str(result.get("id", metadata["Injection ID"]) or metadata["Injection ID"])
            sample_info = result.get("sample_info", {})
            if isinstance(sample_info, dict):
                metadata["Sample ID"] = str(sample_info.get("sample_id", "") or "")

    return metadata


def concern_description(concern: Dict[str, Any]) -> str:
    """Return the English translation text for a concern, with a stable fallback."""
    translations = concern.get("translations", {}) if isinstance(concern, dict) else {}
    if isinstance(translations, dict):
        text = translations.get("en", None)
        if text is not None and str(text).strip():
            return str(text).strip()
    return "No description provided."


def build_analyte_concern_overview_records(
    readable_files: List[str],
    folder: Optional[str],
    analyte: Optional[str],
    data_by_name: Optional[Dict[str, dict]] = None,
) -> List[Dict[str, Any]]:
    """Build one export-friendly row per non-accept concern, with a no-concern row per injection when none are present."""
    rows: List[Dict[str, Any]] = []

    for seq, fn in enumerate(readable_files, start=1):
        data = get_json_from_active_dataset(fn, folder=folder, data_by_name=data_by_name)
        metadata = extract_injection_metadata(data, fn, analyte)

        aqm = data.get("analyte_quad_map", {}) if isinstance(data, dict) else {}
        quad = aqm.get(analyte, None) if isinstance(aqm, dict) and analyte else None

        analyte_present = isinstance(quad, dict)
        concerns = quad.get("concerns", []) if analyte_present else []

        non_accept: List[Dict[str, Any]] = []
        if isinstance(concerns, list):
            for c in concerns:
                if isinstance(c, dict) and normalize_concern_category(c.get("category", None)) != "ACCEPT":
                    non_accept.append(c)
        ...
        base = {
            ...
            "File": fn,
            "Analyte": analyte or "",
            "Analyte Present": "Yes" if analyte_present else "No",
            **metadata,
        }
        ...
        if not non_accept:
            rows.append({
                **base,
                "Concern #": "",
                "Concern Category": "None" if analyte_present else "ANALYTE NOT PRESENT",
                "Raw Category": "",
                "Stage": "",
                "ID": "",
                "Description": "No non-accept concerns to display." if analyte_present else "Selected analyte is not present in this injection.",
            })
            continue

        for concern_index, c in enumerate(non_accept, start=1):
            rows.append({
                **base,
                "Concern #": concern_index,
                "Concern Category": normalize_concern_category(c.get("category", None)),
                "Raw Category": str(c.get("category", "") or ""),
                "Stage": format_max_3dp(c.get("stage", "")),
                "ID": format_max_3dp(c.get("id", "")),
                "Description": concern_description(c),
            })
...
def reset_injection_scoped_display_state() -> None:
    """Reset controls that should return to defaults when the selected injection changes."""
    st.session_state.show_methods = False
    st.session_state.trace_display_mode = "Smoothed"
...
st.sidebar.radio(
    "Page",
    ["Dataset", "Injection / Quad View", "Summary / Trend", "Ion Ratio Analysis", "Peak Shape Analysis", "Concerns Overview"],
    key="page",
)
...
if st.session_state.page == "Dataset":
    ...
    if not files:
        ...
        # Main injection table contains readable injections only. Unreadable files are
        # shown separately below so they cannot be accidentally selected for navigation.
        for seq, fn in enumerate(readable_files, start=1):
            with st.container(border=True):
...
# -----------------------------
# Concerns Overview page
# -----------------------------
elif st.session_state.page == "Concerns Overview":
    st.title("Concerns Overview")
    ...
    st.caption("One row is shown for each non-accept concern. Injections with no non-accept concerns are retained as explicit no-concern rows to preserve sequence fidelity for later export.")

    records = build_analyte_concern_overview_records(readable_files, folder, analyte, data_by_name=data_by_name)
    concerns_df = pd.DataFrame(records)

    if concerns_df.empty:
        st.info("No injection records are available for the selected analyte.")
        st.stop()

    hide_none_concerns = st.checkbox(
        "Hide rows where Concern is None",
        value=False,
        key="concerns_overview_hide_none",
        help="Removes explicit no-concern rows from the displayed table and from the CSV export. Rows where the analyte is not present are retained.",
    )

    displayed_concerns_df = concerns_df.copy()
    if hide_none_concerns:
        displayed_concerns_df = displayed_concerns_df[displayed_concerns_df["Concern Category"] != "None"].copy()

    summary_cols = st.columns(5)
    non_accept_count = int((~displayed_concerns_df["Concern Category"].isin(["None", "ANALYTE NOT PRESENT"])).sum())
    injections_with_non_accept = displayed_concerns_df[~displayed_concerns_df["Concern Category"].isin(["None", "ANALYTE NOT PRESENT"])]
    summary_cols[0].metric("Readable injections", len(readable_files))
    summary_cols[1].metric("Displayed rows", len(displayed_concerns_df))
    summary_cols[2].metric("Concern rows", non_accept_count)
    summary_cols[3].metric("Injections with concerns", injections_with_non_accept["#"].nunique() if not injections_with_non_accept.empty else 0)
    summary_cols[4].metric("Analyte missing", int((displayed_concerns_df["Analyte Present"] == "No").sum()))

    export_suffix = "filtered" if hide_none_concerns else "all_rows"
    st.download_button(
        "Download displayed concerns table as CSV",
        data=displayed_concerns_df.to_csv(index=False).encode("utf-8"),
        file_name=f"concerns_overview_{str(analyte).replace(' ', '_')}_{export_suffix}.csv",
        mime="text/csv",
        help="Exports exactly the rows currently displayed in the Concerns Overview table, preserving one row per concern for later Excel conversion.",
    )

    if displayed_concerns_df.empty:
        st.info("No rows match the current Concerns Overview filter.")
        st.stop()
    ...
    with st.container(border=True):
        header_cols = st.columns([0.55, 1.75, 1.35, 1.1, 1.25, 1.15, 1.15, 1.0, 0.8, 2.9])
        for col, label in zip(
            header_cols,
            ["#", "Injection ID", "Sample ID", "Status", "Software", "Human", "Concern", "Stage", "ID", "Description"],
        ):
            col.markdown(f"**{label}**")

    for row_idx, row in displayed_concerns_df.iterrows():
        seq = int(row["#"])
        fn = str(row["File"])
        concern_category = str(row["Concern Category"])
        concern_col = concern_color(concern_category) if concern_category not in ["None", "ANALYTE NOT PRESENT"] else "#616161"
        status_txt = str(row["Injection Status"])
        software_txt = str(row["Software Opinion"])
        human_txt = str(row["Human Opinion"])
        ...
        with st.container(border=True):
            ...
            row_cols[2].markdown(f"<div class='concerns-row-text'>{html.escape(str(row['Sample ID'])) if str(row['Sample ID']) else '&nbsp;'}</div>", unsafe_allow_html=True)
            row_cols[3].markdown(f"<div class='concerns-row-text' style='color:{status_color(status_txt)}; font-weight:700;'>{status_txt}</div>", unsafe_allow_html=True)
            row_cols[4].markdown(f"<div class='concerns-row-text' style='color:{opinion_color(software_txt)}; font-weight:700;'>{software_txt}</div>", unsafe_allow_html=True)
            row_cols[5].markdown(f"<div class='concerns-row-text' style='color:{human_opinion_color(human_txt)}; font-weight:700;'>{human_txt}</div>", unsafe_allow_html=True)
            row_cols[6].markdown(f"<div class='concerns-row-text' style='color:{concern_col}; font-weight:800;'>{concern_category}</div>", unsafe_allow_html=True)
            row_cols[7].markdown(f"<div class='concerns-row-text'>{html.escape(str(row['Stage'])) if str(row['Stage']) else '&nbsp;'}</div>", unsafe_allow_html=True)
            row_cols[8].markdown(f"<div class='concerns-row-text'>{html.escape(str(row['ID'])) if str(row['ID']) else '&nbsp;'}</div>", unsafe_allow_html=True)
...
# -----------------------------
    ...
    with ctrl1:
        ...
        FWHM: Calculated from apex trace values using baseline-corrected half-height (not smoothed chromatogram data)                     
        RT: Derived from peak.rt                             
        Asymmetry: Calculated using apex trace values at 5% peak height (not smoothed chromatogram data)
        """)
